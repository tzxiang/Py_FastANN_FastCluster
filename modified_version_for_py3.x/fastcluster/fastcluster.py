...
def kmeans(clst_fn,
    ...
    getattr(libfastcluster, "kmeans_" + get_suffix(dtype))\
       (load_rows_func,
        ...
        ctypes.c_int(0),
        ctypes.c_char_p(chkpnt_fn.encode('utf-8')))  # chkpnt_fn.encode('utf-8') / c_wchar_p(chkpnt_fn) / b'***'
...
if __name__ == "__main__":
    ...
    # hdf5_data = hdf5_file.root.clusters  # data name is'clusters', access it by .root.data
    nodes = hdf5_file.list_nodes('/')
    if len(nodes) == 1:
        print("Only 1 group data.")
    hdf5_data = nodes[0]
    print(hdf5_data.shape)
    print(hdf5_data[:10])  # read and process data slices as reading from memory
    hdf5_file.close()
